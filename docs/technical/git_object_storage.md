...
All modern client environments now rely on the Event-Sourced Materializer. (Legacy integrations embedded `isomorphic-git`).
- Artifact documents are abstracted and treated internally as localized virtual repositories.
- `GitRemoteSync` orchestrates the translation between internal event edits and event-stream push/pull operations under the hood.
...
When a legacy client executes a `git push`, the local client constructs a highly optimized `PACK v2` packfile (interleaved with OFS and REF deltas) and transmits it to the worker.
...
1. The local client initiates a `git-upload-pack` HTTP trigger conveying the "want" SHAs (what it needs) alongside "have" SHAs (the outer boundary of what it currently possesses).
