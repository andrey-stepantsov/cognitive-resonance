...
import { Send, BrainCircuit, Activity, Network, Loader2, X, Download, Copy, Check, AlertTriangle, Paperclip, FileText, Diamond, Plus, Trash2, Star, Edit3, Database, Mic, MicOff } from 'lucide-react';
        ...
        {!isViewMode && (
          <div className="p-4 bg-zinc-900/50 border-t border-zinc-800/50 flex flex-col gap-2 relative z-20">
            ...
            {attachedFiles.length > 0 && (
              ...
              {input.startsWith('/') && (
                <div className="absolute bottom-full left-0 mb-2 w-full max-w-sm bg-zinc-900 border border-zinc-700/50 rounded-xl shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-bottom-2">
                  <div className="px-3 py-2 bg-zinc-800/50 border-b border-zinc-700/50 text-xs font-semibold text-zinc-400">
                    System Commands
                  </div>
                  <div className="max-h-48 overflow-y-auto p-1">
                    {[
                      { cmd: '/session ls', desc: 'List recent sessions' },
                      { cmd: '/session new', desc: 'Start a new session' },
                      { cmd: '/clear', desc: 'Clear the current chat view' },
                      { cmd: '/model use [name]', desc: 'Switch active model' },
                      { cmd: '/gem ls', desc: 'List available gems' },
                      { cmd: '/graph ls', desc: 'List active semantic nodes' },
                      { cmd: '/graph stats', desc: 'View graph metrics' }
                    ].filter(c => c.cmd.includes(input) || input === '/').map((c, i) => (
                      <div key={i} className="flex flex-col px-3 py-2 hover:bg-zinc-800 rounded-lg cursor-pointer transition-colors"
                           onClick={() => setInput(c.cmd.replace(' [name]', ' '))}>
                        <span className="text-sm font-mono text-indigo-300">{c.cmd}</span>
