...
exports.BUILT_IN_GEMS = void 0;
exports.default = App;
const react_1 = require("react");
const genai_1 = require("@google/genai");
const lucide_react_1 = require("lucide-react");
const ui_1 = require("@cr/ui");
...
const responseSchema = {
    ...
    properties: {
        reply: { type: genai_1.Type.STRING, description: "The conversational reply to the user." },
        dissonanceScore: { type: genai_1.Type.NUMBER, description: "Cognitive dissonance score (0-100). 0 = absolute certainty, 100 = complete contradiction/confusion." },
        dissonanceReason: { type: genai_1.Type.STRING, description: "Brief explanation of the current dissonance score." },
        semanticNodes: {
            ...
            items: {
                ...
                properties: {
                    ...
                    weight: { type: genai_1.Type.NUMBER, description: "1-10" }
                }
            }
        },
        semanticEdges: {
...
function App() {
    ...
    const [selectedTurnIndex, setSelectedTurnIndex] = (0, react_1.useState)(null);
    // Session State
    const [sessions, setSessions] = (0, react_1.useState)([]);
    const [activeSessionId, setActiveSessionId] = (0, react_1.useState)(null);
    const [isHistorySidebarOpen, setIsHistorySidebarOpen] = (0, react_1.useState)(false);
    const [historySearchQuery, setHistorySearchQuery] = (0, react_1.useState)('');
    const [isHistorySearchActive, setIsHistorySearchActive] = (0, react_1.useState)(false);
    const [activeSidebarTab, setActiveSidebarTab] = (0, react_1.useState)('history');
    const [searchResults, setSearchResults] = (0, react_1.useState)([]);
    const [targetTurnIndex, setTargetTurnIndex] = (0, react_1.useState)(null);
    const [editingSessionId, setEditingSessionId] = (0, react_1.useState)(null);
    const [editSessionName, setEditSessionName] = (0, react_1.useState)('');
    const [markerViewMode, setMarkerViewMode] = (0, react_1.useState)('graph');
    const [markerSearchQuery, setMarkerSearchQuery] = (0, react_1.useState)('');
    // Layout State
    const [isDissonancePanelOpen, setIsDissonancePanelOpen] = (0, react_1.useState)(false);
    const [isRightSidebarOpen, setIsRightSidebarOpen] = (0, react_1.useState)(false);
    const [copiedIndex, setCopiedIndex] = (0, react_1.useState)(null);
    const [isGemSidebarOpen, setIsGemSidebarOpen] = (0, react_1.useState)(false);
    ...
    const [savedGems, setSavedGems] = (0, react_1.useState)(exports.BUILT_IN_GEMS);
    const [defaultGemId, setDefaultGemId] = (0, react_1.useState)('gem-general');
    const [activeGemId, setActiveGemId] = (0, react_1.useState)('gem-general');
    const [selectedModel, setSelectedModel] = (0, react_1.useState)('gemini-2.5-flash');
    // Single system prompt state for the session (derived from gem at start, or if user edits)
    const [sessionSystemPrompt, setSessionSystemPrompt] = (0, react_1.useState)(exports.BUILT_IN_GEMS[0].systemPrompt);
    const [editingGem, setEditingGem] = (0, react_1.useState)(null);
    const [creatingGem, setCreatingGem] = (0, react_1.useState)(false);
    const [draftGem, setDraftGem] = (0, react_1.useState)({ name: '', model: 'gemini-2.5-flash', systemPrompt: '' });
    const [isViewMode, setIsViewMode] = (0, react_1.useState)(false);
    const [historyFilename, setHistoryFilename] = (0, react_1.useState)('');
    ...
    (0, react_1.useEffect)(() => {
        const handleMessage = (event) => {
            ...
            if (message.type === 'response') {
                ...
                const userGems = list.filter((g) => !g.isBuiltIn && g.id !== 'gem-general' && g.id !== 'gem-coder');
                const finalGems = [...exports.BUILT_IN_GEMS, ...userGems];
                ...
                if (message.data.config) {
                    ...
                    if (message.data.config.gemId)
                        setActiveGemId(message.data.config.gemId);
                }
                setIsLoading(false);
            }
    ...
    const handleSaveGem = (gemProfile) => {
    ...
    const handleSetDefaultGem = (id, e) => {
        e.stopPropagation();
        setDefaultGemId(id);
    ...
    const startRenameSession = (sessionId, currentName, e) => {
        e.stopPropagation();
        setEditingSessionId(sessionId);
        setEditSessionName(currentName);
    };
    const handleRenameSessionSubmit = (sessionId, e) => {
        e.stopPropagation();
        if (e.type === 'submit') {
            e.preventDefault();
        }
    ...
    return (<div className="flex flex-col h-screen bg-[#111116] text-zinc-100 font-sans overflow-hidden">
      
      {/* Session Sidebar Backdrop */}
      {isHistorySidebarOpen && (<div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-40" onClick={() => setIsHistorySidebarOpen(false)}/>)}

      {/* Session Sidebar Options Panel */}
      <div className={cn("fixed top-0 left-0 bottom-0 w-[280px] bg-zinc-900 border-r border-zinc-800/50 shadow-2xl z-50 transform transition-transform duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] flex flex-col", isHistorySidebarOpen ? "translate-x-0" : "-translate-x-full")}>
        <div className="p-4 border-b border-zinc-800 flex justify-between items-center">
          <div className="flex gap-4">
            <button onClick={() => setActiveSidebarTab('history')} className={cn("text-sm font-semibold tracking-wide transition-colors pb-1 border-b-2", activeSidebarTab === 'history' ? "text-zinc-200 border-indigo-500" : "text-zinc-500 border-transparent hover:text-zinc-300")}>
              History
            </button>
            <button onClick={() => { setActiveSidebarTab('search'); setHistorySearchQuery(''); }} className={cn("text-sm font-semibold tracking-wide transition-colors pb-1 border-b-2 flex items-center gap-1.5", activeSidebarTab === 'search' ? "text-zinc-200 border-indigo-500" : "text-zinc-500 border-transparent hover:text-zinc-300")}>
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
              Search
            </button>
          </div>
          <button onClick={() => setIsHistorySidebarOpen(false)} className="text-zinc-500 hover:text-white transition-colors p-1">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/></svg>
          </button>
        </div>

        {activeSidebarTab === 'search' && (<div className="p-3 border-b border-zinc-800/50 bg-zinc-900/50">
            <div className="relative">
              <input type="text" placeholder="Search concepts across all sessions..." value={historySearchQuery} onChange={(e) => setHistorySearchQuery(e.target.value)} autoFocus className="w-full bg-zinc-950/80 border border-zinc-700/50 rounded-lg py-2 pl-3 pr-3 text-sm text-zinc-200 placeholder-zinc-500 focus:outline-none focus:border-indigo-500/80 transition-colors shadow-inner"/>
            </div>
          </div>)}

        {activeSidebarTab === 'history' && (<div className="p-3">
            <button onClick={startNewSession} className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-indigo-600/10 hover:bg-indigo-600/20 text-indigo-400 border border-indigo-500/20 rounded-lg text-sm font-medium transition-all">
        ...
        <div className="flex-1 overflow-y-auto px-3 pb-4 space-y-1 mt-1">
          {activeSidebarTab === 'history' && sessions.length === 0 && (<div className="text-xs text-zinc-500 text-center mt-6">No previous sessions found</div>)}
          {activeSidebarTab === 'history' && sessions.map(s => (<div key={s.id} onClick={() => {
                if (editingSessionId !== s.id) {
                    handleLoadSession(s.id);
                }
            }} className={cn("group relative px-3 py-2.5 rounded-lg transition-colors border border-transparent flex justify-between items-center", editingSessionId !== s.id && "cursor-pointer", activeSessionId === s.id
                ? "bg-zinc-800/80 border-zinc-700/50 text-indigo-300"
                : "hover:bg-zinc-800/50 text-zinc-400 hover:text-zinc-200")}>
              {editingSessionId === s.id ? (<div className="flex items-center gap-2 w-full" onClick={e => e.stopPropagation()}>
                  <input type="text" value={editSessionName} onChange={(e) => setEditSessionName(e.target.value)} onKeyDown={(e) => {
                    if (e.key === 'Enter')
                        handleRenameSessionSubmit(s.id, e);
                    if (e.key === 'Escape')
                        setEditingSessionId(null);
                  ...
                  <div className="opacity-0 group-hover:opacity-100 flex items-center gap-1 transition-all shrink-0">
                    <button onClick={(e) => startRenameSession(s.id, s.customName || s.preview, e)} className="p-1.5 text-zinc-500 hover:text-indigo-400 hover:bg-indigo-500/10 rounded-md transition-colors" title="Rename Session">
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                    </button>
                    <button onClick={(e) => handleDeleteSession(s.id, e)} className="p-1.5 text-red-500/70 hover:text-red-400 hover:bg-red-500/10 rounded-md transition-colors" title="Delete Session">
          ...
          {activeSidebarTab === 'search' && historySearchQuery.trim() === '' && (<div className="text-xs text-zinc-500 text-center mt-6 px-4 leading-relaxed">
               Type a concept to search your entire Cognitive Resonance history index.
             </div>)}
          
          {activeSidebarTab === 'search' && historySearchQuery.trim() !== '' && searchResults.length === 0 && (<div className="text-xs text-zinc-500 text-center mt-6">No matching concepts found.</div>)}
          
          {activeSidebarTab === 'search' && searchResults.map((r, i) => (<div key={`${r.sessionId}-${r.turnIndex}-${i}`} onClick={() => handleSearchResultClick(r)} className={cn("group relative p-3 rounded-lg cursor-pointer transition-colors border border-transparent flex flex-col gap-1.5", "hover:bg-zinc-800/60 bg-zinc-900/30 text-zinc-300 hover:border-zinc-700")}>
              <div className="flex flex-wrap gap-1">
                 {r.matchedConcepts.map((c) => (<span key={c} className="px-1.5 py-0.5 bg-indigo-500/20 text-indigo-300 rounded text-[10px] font-medium border border-indigo-500/30">
                      {c}
                    </span>))}
              </div>
      ...
      <header className="flex-none px-6 py-4 border-b border-zinc-800/50 bg-zinc-900/30 flex items-center justify-between backdrop-blur-md sticky top-0 z-20">
        <div className="flex items-center gap-3">
          <button onClick={() => setIsHistorySidebarOpen(true)} className="p-1.5 text-zinc-400 hover:text-indigo-400 bg-zinc-800/30 hover:bg-zinc-800 rounded-md transition-colors" title="Session History">
          ...
          <h1 className="text-sm font-semibold tracking-wide text-zinc-100 flex items-center gap-2">
            ...
            {activeState?.tokenUsage && (<span className="flex items-center gap-1.5 px-2 py-0.5 bg-indigo-500/10 text-indigo-300 rounded text-[10px] font-mono border border-indigo-500/20 shadow-[0_0_8px_rgba(99,102,241,0.15)] ml-2 transition-all">
                <lucide_react_1.Database className="w-3 h-3 text-indigo-400"/>
        ...
        <div className="flex items-center gap-2">
          {!isViewMode && (<>
              <button onClick={handleDownloadHistory} disabled={messages.length === 0} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-zinc-400 disabled:opacity-30 disabled:cursor-not-allowed hover:text-white bg-zinc-800/30 hover:bg-zinc-800 rounded border border-zinc-800 transition-colors" title="Download Snapshot JSON">
                <lucide_react_1.Download className="w-3.5 h-3.5"/>
                Backup
              </button>
            </>)}
        </div>
      ...
      {/* Mobile Backdrop */}
      {isDissonancePanelOpen && (<div className="fixed inset-0 bg-black/60 z-40 lg:hidden backdrop-blur-sm" onClick={() => { setIsDissonancePanelOpen(false); }}/>)}

      {/* Left Sidebar: Dissonance */}
      <div className={cn("fixed inset-y-0 left-0 z-50 w-[85vw] sm:w-80 bg-zinc-950 lg:bg-zinc-900/30 border-r border-zinc-800/50 flex flex-col p-6", "transform transition-transform duration-300 ease-in-out lg:relative lg:transform-none lg:z-auto", isDissonancePanelOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0")}>
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            <lucide_react_1.Activity className="w-5 h-5 text-indigo-400"/>
            <h2 className="font-medium tracking-wide text-zinc-100">Internal State</h2>
          </div>
          <div className="flex items-center gap-2">
            {isViewingHistory && (<button onClick={() => setSelectedTurnIndex(null)} className="text-xs bg-indigo-500/20 text-indigo-300 px-2.5 py-1.5 rounded-md hover:bg-indigo-500/30 transition-colors">
                Return to Current
              </button>)}
        ...
        <ui_1.DissonanceMeter currentScore={activeState?.dissonanceScore ?? null} reason={activeState?.dissonanceReason ?? null} history={historyData} activeTurnIndex={activeTurnIndex} isViewingHistory={isViewingHistory} onSelectTurn={setSelectedTurnIndex}/>
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 w-full lg:min-w-[400px] max-w-3xl mx-auto lg:border-x border-zinc-800/30 bg-[#0a0a0a] shadow-2xl z-10">
        <div className="p-4 lg:p-6 border-b border-zinc-800/50 flex items-center justify-between bg-zinc-900/20 backdrop-blur-md relative">
          ...
          <div className="flex items-center gap-3 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
            <lucide_react_1.BrainCircuit className="w-6 h-6 text-indigo-500"/>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {messages.length === 0 && (<div className="h-full flex flex-col items-center justify-center text-zinc-500 space-y-4 px-8 text-center">
              <lucide_react_1.BrainCircuit className="w-12 h-12 opacity-20 mb-2"/>
          ...
          {messages.map((msg, idx) => (<div key={idx} id={`message-${idx}`} className={cn("flex w-full flex-col scroll-mt-24 min-w-0 break-words", msg.role === 'user' ? "items-end" : "items-start")}>
              {msg.isError ? (<div className="max-w-[80%] rounded-2xl px-5 py-3.5 text-sm leading-relaxed bg-red-950/60 text-red-200 border border-red-800/60 rounded-bl-sm">
                  <div className="flex items-start gap-2.5">
                    <lucide_react_1.AlertTriangle className="w-4 h-4 text-red-400 mt-0.5 shrink-0"/>
                  ...
                  <button onClick={() => {
                    ...
                    {copiedIndex === idx ? (<><lucide_react_1.Check className="w-3 h-3"/> Copied!</>) : (<><lucide_react_1.Copy className="w-3 h-3"/> Copy error to clipboard</>)}
                  </button>
                </div>) : (<div className={cn("max-w-[85%] rounded-2xl px-5 py-3.5 text-sm leading-relaxed overflow-hidden break-words min-w-0", msg.role === 'user'
                  ...
                  {msg.role === 'model' && !msg.isError ? (<ui_1.MarkdownRenderer content={msg.content}/>) : (msg.content)}
                </div>)}
              {msg.role === 'model' && msg.modelTurnIndex !== undefined && !msg.isError && (<button onClick={() => {
                    setSelectedTurnIndex(msg.modelTurnIndex);
                    setIsDissonancePanelOpen(true);
                }} className={cn("mt-2 text-xs font-medium transition-colors flex items-center gap-1.5 px-1", activeTurnIndex === msg.modelTurnIndex
                    ? "text-indigo-400"
                    : "text-zinc-500 hover:text-zinc-300")}>
                  <lucide_react_1.Activity className="w-3.5 h-3.5"/>
          ...
          {isLoading && (<div className="flex justify-start">
              <div className="bg-zinc-800/80 border border-zinc-700/50 rounded-2xl rounded-bl-sm px-5 py-4 flex items-center gap-3">
                <lucide_react_1.Loader2 className="w-4 h-4 animate-spin text-indigo-400"/>
        ...
        {!isViewMode && (<div className="p-4 bg-zinc-900/50 border-t border-zinc-800/50 flex flex-col gap-2 relative z-20">
            {/* Prompt Area Controls */}
            <div className="flex items-center gap-2 px-1 pb-1">
              <button onClick={() => setIsGemSidebarOpen(true)} className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium bg-zinc-800/40 hover:bg-zinc-800 text-indigo-300 border border-indigo-500/20 rounded-lg transition-colors shadow-sm" title="Manage Gems">
                <lucide_react_1.Diamond className="w-3.5 h-3.5"/>
                {savedGems.find(g => g.id === activeGemId)?.name || 'Select Gem'}
              </button>
              <select value={selectedModel} onChange={(e) => setSelectedModel(e.target.value)} className={cn("text-xs font-medium bg-transparent hover:bg-zinc-800/40 border border-transparent hover:border-zinc-700/50 rounded-lg px-2 py-1.5 cursor-pointer focus:outline-none transition-all max-w-[200px] truncate shadow-sm", (!selectedModel || !chatModels.find(m => m.name.replace('models/', '') === selectedModel.replace('models/', ''))) ? 'text-red-400/90' : 'text-zinc-400')} title="Override model for this session">
                {chatModels.length === 0 && (<option value={selectedModel}>{selectedModel}</option>)}
                {chatModels.length > 0 && (!selectedModel || !chatModels.find(m => m.name.replace('models/', '') === selectedModel.replace('models/', ''))) && (<option value={selectedModel || ''} className="text-red-500" disabled>Select valid chat model...</option>)}
                {chatModels.map((m) => {
            ...
            {attachedFiles.length > 0 && (<div className="flex flex-wrap gap-2 px-1">
                {attachedFiles.map(f => (<div key={f.id} className="flex items-center gap-2 bg-zinc-800/80 border border-zinc-700/50 rounded-lg px-2.5 py-1.5 text-xs group animate-in fade-in slide-in-from-bottom-2 duration-200">
                    {f.preview ? (<img src={f.preview} alt={f.name} className="w-8 h-8 rounded object-cover"/>) : (<lucide_react_1.FileText className="w-4 h-4 text-zinc-400"/>)}
                    <span className="text-zinc-300 max-w-[120px] truncate">{f.name}</span>
                    <button onClick={() => setAttachedFiles(prev => prev.filter(af => af.id !== f.id))} className="p-0.5 text-zinc-500 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100">
                      <lucide_react_1.X className="w-3 h-3"/>
                    </button>
                  </div>))}
              </div>)}
            <form onSubmit={handleSubmit} className="relative flex items-center">
              {(!selectedModel || (chatModels.length > 0 && !chatModels.find(m => m.name.replace('models/', '') === selectedModel.replace('models/', '')))) && (<div className="absolute -top-10 left-0 w-full text-center pointer-events-none z-50">
              ...
              <button type="button" onClick={() => vscode.postMessage({ type: 'request_file_selection' })} disabled={isLoading} className="p-2.5 text-zinc-400 hover:text-indigo-400 transition-colors disabled:opacity-40 shrink-0" title="Attach files">
                <lucide_react_1.Paperclip className="w-4 h-4"/>
              </button>
              <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Send a message..." disabled={isLoading || !selectedModel || (chatModels.length > 0 && !chatModels.find(m => m.name.replace('models/', '') === selectedModel.replace('models/', '')))} className="w-full bg-zinc-950 border border-zinc-700/50 rounded-xl pl-4 pr-12 py-3.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all disabled:opacity-50"/>
              <button type="submit" disabled={!input.trim() || isLoading || !selectedModel || (chatModels.length > 0 && !chatModels.find(m => m.name.replace('models/', '') === selectedModel.replace('models/', '')))} className="absolute right-2 p-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg transition-colors disabled:opacity-50 disabled:hover:bg-indigo-600">
      ...
      <div className={cn("fixed inset-y-0 right-0 z-50 w-[85vw] sm:w-96 bg-zinc-950 lg:bg-zinc-900/30 border-l border-zinc-800/50 flex flex-col p-6", "transform transition-transform duration-300 ease-in-out lg:relative lg:transform-none lg:z-auto", isRightSidebarOpen ? "translate-x-0" : "translate-x-full lg:translate-x-0")}>
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            <lucide_react_1.Network className="w-5 h-5 text-indigo-400"/>
          ...
          <div className="flex items-center gap-2">
            <div className="flex bg-zinc-900/80 rounded-lg p-0.5 border border-zinc-800">
              <button onClick={() => setMarkerViewMode('graph')} className={cn("px-2.5 py-1 text-xs font-medium rounded-md transition-all", markerViewMode === 'graph' ? "bg-zinc-700/50 text-indigo-300 shadow-sm" : "text-zinc-500 hover:text-zinc-300")}>
                Graph
              </button>
              <button onClick={() => setMarkerViewMode('list')} className={cn("px-2.5 py-1 text-xs font-medium rounded-md transition-all", markerViewMode === 'list' ? "bg-zinc-700/50 text-indigo-300 shadow-sm" : "text-zinc-500 hover:text-zinc-300")}>
                List
              </button>
            </div>
            {isViewingHistory && (<button onClick={() => setSelectedTurnIndex(null)} className="text-xs bg-indigo-500/20 text-indigo-300 px-2.5 py-1.5 rounded-md hover:bg-indigo-500/30 transition-colors">
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          {markerViewMode === 'graph' ? (<div className="flex-1 min-h-0 relative">
                  <ui_1.SemanticGraph nodes={activeState?.semanticNodes ?? []} edges={activeState?.semanticEdges ?? []} onNodeClick={(nodeId) => {
                // Find the first message index where this semantic node appears in the internalState
                const targetIdx = messages.findIndex(m => m.internalState?.semanticNodes?.some(n => n.id === nodeId));
                if (targetIdx !== -1) {
                ...
                <input type="text" placeholder="Filter markers..." value={markerSearchQuery} onChange={(e) => setMarkerSearchQuery(e.target.value)} className="w-full bg-zinc-900/50 border border-zinc-700/50 rounded-lg px-3 py-2 text-sm text-zinc-200 focus:outline-none focus:border-indigo-500/80 mb-3 shrink-0"/>
                <div className="flex-1 overflow-y-auto pr-2 space-y-1">
                  {filteredMarkers.length === 0 && (<div className="text-zinc-500 text-xs text-center py-4">No markers found.</div>)}
                  {filteredMarkers.map(m => (<div key={m.name} onClick={() => {
      ...
      {isGemSidebarOpen && (<div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-40" onClick={() => setIsGemSidebarOpen(false)}/>)}
      <div className={cn("fixed top-0 right-0 bottom-0 w-[340px] bg-zinc-900 border-l border-zinc-800/50 shadow-2xl z-50 transform transition-transform duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] flex flex-col", isGemSidebarOpen ? "translate-x-0" : "translate-x-full")}>
        <div className="p-4 border-b border-zinc-800 flex justify-between items-center">
          <div className="flex items-center gap-2">
            <lucide_react_1.Diamond className="w-4 h-4 text-indigo-400"/>
            <h2 className="text-sm font-semibold tracking-wide text-zinc-200">Gems</h2>
          </div>
          <button onClick={() => { setIsGemSidebarOpen(false); setEditingGem(null); setCreatingGem(false); }} className="text-zinc-500 hover:text-white transition-colors p-1">
            <lucide_react_1.X className="w-4 h-4"/>
          </button>
        </div>

        {editingGem || creatingGem ? (<div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 animate-in slide-in-from-right-2 duration-200">
              <div className="flex items-center gap-2 text-sm font-medium text-zinc-300 mb-2">
                <button onClick={() => { setEditingGem(null); setCreatingGem(false); }} className="text-zinc-500 hover:text-zinc-300"><lucide_react_1.X className="w-4 h-4"/></button>
                {creatingGem ? 'New Custom Gem' : 'Edit Gem'}
              </div>
              <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-zinc-400 ml-1">Name</label>
                  <input type="text" value={editingGem ? editingGem.name : draftGem.name || ''} onChange={(e) => {
                if (editingGem)
                    setEditingGem({ ...editingGem, name: e.target.value });
                else
                    setDraftGem({ ...draftGem, name: e.target.value });
              ...
              <div className="space-y-1.5">
                  ...
                  <select value={editingGem ? editingGem.model : draftGem.model || 'gemini-2.5-flash'} onChange={(e) => {
                if (editingGem)
                    setEditingGem({ ...editingGem, model: e.target.value });
                else
                    setDraftGem({ ...draftGem, model: e.target.value });
              ...
              <div className="space-y-1.5 flex-1 flex flex-col">
                  ...
                  <textarea value={editingGem ? editingGem.systemPrompt : draftGem.systemPrompt || ''} onChange={(e) => {
                if (editingGem)
                    setEditingGem({ ...editingGem, systemPrompt: e.target.value });
                else
                    setDraftGem({ ...draftGem, systemPrompt: e.target.value });
            ...
            }} disabled={editingGem ? !editingGem.name : !draftGem.name} className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium rounded-lg transition-colors disabled:opacity-50">
                    Save Gem
                 </button>
              </div>
           </div>) : (<>
              ...
              <div className="flex-1 overflow-y-auto px-3 pb-4 space-y-2 mt-1">
                {savedGems.map(g => (<div key={g.id} onClick={() => handleSelectGem(g.id)} className={cn("group relative px-3 py-3 rounded-lg cursor-pointer transition-colors border flex flex-col gap-1", activeGemId === g.id
                    ...
                    : "bg-zinc-800/20 border-zinc-800/80 hover:bg-zinc-800/50")}>
                     <div className="flex items-start justify-between">
                         ...
                         <div className="flex items-center gap-1 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                             <button onClick={(e) => handleSetDefaultGem(g.id, e)} className={cn("p-1.5 rounded-md transition-colors", defaultGemId === g.id ? "text-amber-400" : "text-zinc-500 hover:text-amber-400 hover:bg-amber-400/10")} title="Set as Default">
                               <lucide_react_1.Star className="w-3.5 h-3.5" fill={defaultGemId === g.id ? "currentColor" : "none"}/>
                             </button>
                             {!g.isBuiltIn && (<>
                                 <button onClick={(e) => { e.stopPropagation(); setEditingGem(g); }} className="p-1.5 text-zinc-400 hover:text-white hover:bg-zinc-700 rounded-md transition-colors">
                                   <lucide_react_1.Edit3 className="w-3.5 h-3.5"/>
                                 </button>
                                 <button onClick={(e) => handleDeleteGem(g.id, e)} className="p-1.5 text-red-500/70 hover:text-red-400 hover:bg-red-500/10 rounded-md transition-colors">
