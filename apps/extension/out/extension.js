...
function activate(context) {
    ...
    let browseGalleryCommand = vscode.commands.registerCommand('cognitive-resonance.browseGallery', async () => {
        try {
            ...
            const entries = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Loading Gallery...', cancellable: false }, async () => {
            ...
            const data = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Downloading: ${selected.filename}...`, cancellable: false }, async () => {
            ...
            const panel = vscode.window.createWebviewPanel('cognitiveResonanceHistory', `Gallery: ${selected.filename.replace('.json', '')}`, vscode.ViewColumn.One, {
                enableScripts: true,
                retainContextWhenHidden: true,
                localResourceRoots: [vscode.Uri.file(path.join(context.extensionPath, 'webview-ui', 'dist'))]
            });
